...
export function FacultyTaskTab({ subjectId, classId }: { subjectId: number; classId: number }) {
  const router = useRouter();
  const [status, setStatus] = useState<"loading" | "success" | "error">("loading");
  const [tasks, setTasks] = useState<LmsFacultyTask[]>([]);
  ...
  const [taskType, setTaskType] = useState<LmsTaskType>("assignment");
  const [selectedClassIds, setSelectedClassIds] = useState<number[]>([]);
  const [saving, setSaving] = useState(false);

  const load = useCallback(() => {
    setStatus("loading");
    Promise.all([getFacultyTasks(subjectId, classId), getMyTeachingSubjects()])
      .then(([taskData, subjects]: [LmsFacultyTask[], LmsTeachingSubject[]]) => {
        setTasks(taskData);
  ...
  function handleCreate() {
    ...
    const marks = maxMarks.trim() ? Number(maxMarks.trim()) : undefined;
    if (maxMarks.trim() && (!Number.isInteger(marks) || (marks as number) <= 0)) {
      toast.warning("Max marks must be a whole number");
      return;
    }
    ...
    if (dueDate.trim()) {
      ...
      if (Number.isNaN(parsed.getTime())) {
        toast.warning("Due date must look like YYYY-MM-DD");
        return;
      }
    ...
    createTask({
      ...
      .catch((err) => toast.error(getApiErrorMessage(err, "Couldn't create the task.")))
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            <TouchableOpacity style={styles.newButton} onPress={openCreate} activeOpacity={0.9}>
              ...
              <Text style={styles.newButtonText}>Create new task</Text>
            </TouchableOpacity>
            {tasks.length > 0 && (
              <View style={styles.sectionHeaderRow}>
                <Text style={styles.sectionHeaderText}>YOUR TASKS</Text>
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>No tasks created for this class yet</Text>
          </View>
        }
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            onPress={() =>
              router.push({ pathname: "/(tabs)/academics/lms/task/[taskId]", params: { taskId: String(item.id), title: item.title ?? "Task" } })
            }
          >
            ...
            <View style={styles.textWrap}>
              ...
              <Text style={styles.meta}>
                {item.class_label} · {item.submitted_count} submitted{item.max_marks ? ` · ${item.max_marks} marks` : ""}
              </Text>
            </View>
        ...
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setCreateOpen(false)}>
          <TouchableOpacity style={styles.modalCard} activeOpacity={1}>
            ...
            <View style={styles.typeRow}>
              {(["assignment", "quiz"] as LmsTaskType[]).map((type) => (
                <TouchableOpacity
                  key={type}
                  style={[styles.typeChip, taskType === type && styles.typeChipSelected]}
                  onPress={() => setTaskType(type)}
                  activeOpacity={0.85}
                >
                  <Text style={[styles.typeChipText, taskType === type && styles.typeChipTextSelected]}>
                    {type === "assignment" ? "Assignment" : "Quiz"}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            <TextInput style={styles.modalInput} placeholder="Title" placeholderTextColor="#9AA6B2" value={title} onChangeText={setTitle} />
            <TextInput
            ...
            />
            <Text style={styles.modalFieldLabel}>Assign to</Text>
...
const styles = StyleSheet.create({
  ...
  typeChip: { flex: 1, alignItems: "center", paddingVertical: 9, borderRadius: 10, backgroundColor: "#EEF0F4" },
  typeChipSelected: { backgroundColor: "#2F6FE0" },
