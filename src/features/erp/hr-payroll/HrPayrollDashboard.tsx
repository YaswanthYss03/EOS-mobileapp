...
import { hrSectionItems, employeeSectionItems } from "./data/mockDashboard";

function HrPayrollHeader({ onBack }: { onBack: () => void }) {
  ...
  return (
    ...
    >
      ...
      <View>
        ...
        <Text style={headerStyles.subtitle}>HR & Payroll services</Text>
      </View>
    </LinearGradient>
  );
}

// TODO: hr-payroll RBAC - view-only pages for this role go here as siblings/subfolders.
...
export function HrPayrollDashboard() {
  ...
  // restoring the shared one on blur/unmount - same pattern used by the HoD
  // and Employee dashboards.
  useFocusEffect(
    useCallback(() => {
      navigation.getParent()?.setOptions({
        header: () => <HrPayrollHeader onBack={() => router.replace("/(tabs)/home")} />,
      });
      return () => {
    ...
    <SafeAreaView style={styles.container} edges={[]}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.section}>
          ...
          <QuickAccessGrid items={hrSectionItems} />
        </View>
