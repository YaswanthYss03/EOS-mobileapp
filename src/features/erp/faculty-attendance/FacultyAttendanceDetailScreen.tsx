...
// Wired to GET /me/staff-attendance/:facultyId - drill-down from the HR
// attendance roster (FacultyAttendanceScreen). Same day-status calendar as
// the faculty's own My Attendance screen, but for the faculty member chosen
// on the previous screen instead of the caller. A day with no record in any
// of the three real sources (faculty_daily_attendance, approved leaves,
// opted-in holiday slots) has no entry and simply renders as blank/present-
// styled, matching the calendar's fixed 4-state palette.
export function FacultyAttendanceDetailScreen() {
  ...
  const name = params.name ?? "Faculty";
  const subtitle = params.subtitle ?? "";

  const now = useMemo(() => new Date(), []);
  const [viewYear, setViewYear] = useState(params.year ? Number(params.year) : now.getFullYear());
  const [viewMonth, setViewMonth] = useState(params.month ? Number(params.month) - 1 : now.getMonth());

  const [status, setStatus] = useState<LoadStatus>("loading");
  const [attendance, setAttendance] = useState<FacultyAttendanceDetail | null>(null);
  ...
  const load = useCallback(() => {
    setStatus("loading");
    getStaffAttendanceForFaculty(facultyId, viewYear, viewMonth + 1)
      .then((response) => {
        setAttendance(response);
        setStatus("success");
      })
      .catch(() => setStatus("error"));
  }, [facultyId, viewYear, viewMonth]);
  ...
  function statusFor(day: number): StaffAttendanceDayStatus | null {
    const key = `${viewYear}-${pad2(viewMonth + 1)}-${pad2(day)}`;
    return attendance?.marks[key] ?? null;
  }
    ...
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      {status === "error" && (
        <View style={styles.errorNotice}>
          ...
          <Text style={styles.errorNoticeText}>Couldn't load this faculty member's attendance.</Text>
            ...
            {weeks.map((week, weekIndex) => (
              <View key={weekIndex} style={styles.weekRow}>
                {week.map((day, dayIndex) => {
                  ...
                  const { bg, text } = dayStatus ? STATUS_STYLES[dayStatus] : STATUS_STYLES.present;
                  return (
